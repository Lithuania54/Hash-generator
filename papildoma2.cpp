...
Block mineWithCandidates(const string &prevBlockHash, vector<Transaction> &transactions) {
    ...
    for (auto &candidate : candidates) {
        for (unsigned long long attemptCount = 0; attemptCount < maxAttempts; ++attemptCount) {
            ...
            if (chrono::steady_clock::now() - startTime > maxTime) {
                cout << "Time limit reached. Restarting mining with new candidates." << endl;
                break;
            }
        }
    }

    cerr << "No block mined within the attempt and time limits.\n";
    return Block();
}
